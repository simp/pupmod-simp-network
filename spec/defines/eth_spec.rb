    ...
    on_supported_os.each do |os, os_facts|
      context "on #{os}" do
        ...
        context 'with default parameters' do
          ...
          let!(:expected) do
            ...
            if el_maj_release > '7' ||
              ...
              content.gsub!(%r{^NM_CONTROLLED=\S+}m, 'NM_CONTROLLED=yes')
            end
        ...
        context 'base' do
          ...
          it { is_expected.to create_file('/etc/sysconfig/network-scripts/ifcfg-test_eth').with_content(%r{^BOOTPROTO=dhcp}) }
          it { is_expected.to create_file('/etc/sysconfig/network-scripts/ifcfg-test_eth').with_content(%r{^HWADDR=CA:FE:BA:BE:00:FF}) }
          it { is_expected.to create_file('/etc/sysconfig/network-scripts/ifcfg-test_eth').without_content(%r{^(?!\w|#|\s*$)}) }
        end
        ...
        context 'bond' do
          ...
          it { is_expected.to create_file('/etc/sysconfig/network-scripts/ifcfg-test_bond').with_content(%r{^BONDING_OPTS='miimon=100 mode=4'}) }
          it { is_expected.to create_file('/etc/sysconfig/network-scripts/ifcfg-test_bond').without_content(%r{^(?!\w|#|\s*$)}) }
        end
        ...
        context 'when auto_restart is overridden globally' do
          ...
          context 'when true' do
            ...
            it { is_expected.to create_exec("network_restart_#{title}").with_command(%r{sleep}) }
          end
          ...
          context 'when false' do
            ...
            it { is_expected.to create_exec("network_restart_#{title}").with_command(%r{true}) }
          end
        end
      end
    end
